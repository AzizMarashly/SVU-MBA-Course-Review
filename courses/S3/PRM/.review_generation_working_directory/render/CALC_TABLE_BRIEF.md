# Brief: tables and calculation blocks (§7d) — converting existing records
...
Work only inside W = `C:\Users\root\SVU-MBA-Course-Review\courses\S3\PRM\.review_generation_working_directory\render`.
...
## Symbols glossary (`meta_prm.py` → `SYMBOLS`)
