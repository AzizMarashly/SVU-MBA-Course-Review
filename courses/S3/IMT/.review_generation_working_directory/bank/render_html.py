...
var pgOpen={};try{pgOpen=JSON.parse(localStorage.getItem('imt_pg_open')||'{}');}catch(e){}
document.querySelectorAll('details.pgd').forEach(function(d){var id=d.getAttribute('data-pg');if(pgOpen[id]===true)d.open=true;if(pgOpen[id]===false)d.open=false;
 d.addEventListener('toggle',function(){pgOpen[id]=d.open;try{localStorage.setItem('imt_pg_open',JSON.stringify(pgOpen));}catch(e){}});});
