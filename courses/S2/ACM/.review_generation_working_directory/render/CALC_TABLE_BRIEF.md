# Brief: tables and calculation blocks (§7d) — ACM copy of the PRM brief: write new records in this form
...
Work only inside W = `C:\Users\root\SVU-MBA-Course-Review\courses\S2\ACM\.review_generation_working_directory\render`.
...
## Symbols glossary (`meta_acm.py` → `SYMBOLS`)
...
## ACM additions (prompt v0.12)

- `C(..., method="M1")` names the chapter's `METHODS` entry the steps follow; the step order must equal the entry's order (§7e).
- `F("cvp", price, var, fixed, be_q, cash_fixed=None, sales_q=None, highlight=..., alt=...)` draws the break-even chart from the record's data; the
  build recomputes `fixed ÷ (price − var)` and fails when it differs from `be_q`.
- Book conventions for ACM: CM = P − V; CMR = CM ÷ P; BEQ = FC ÷ CM; BEV = FC ÷ CMR; target quantity = (FC + TP) ÷ CM; shutdown point = cash
  fixed costs ÷ CM (cash fixed = FC − depreciation); margin of safety = Q − BEQ (ratio ÷ Q); straight-line Dep = (Cost − SV) ÷ N;
  sum-of-years fraction = remaining years ÷ SYD; double-declining rate = 2 ÷ N on the book value; asset cost = purchase price + every cost to
  make it ready (freight, installation, customs, commissions).
- Amounts in `sub` cells are plain digits without thousands separators (the reader's exam shows them that way); `res` may use «ل.س», «وحدة», «%».
