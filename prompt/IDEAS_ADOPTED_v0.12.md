# Ideas adopted in prompt v0.12 (archive)

All twelve entries below were moved into the prompt in v0.12 (2026-09-25); each is marked with the section it landed
in. Kept as a record because course logs and code comments cite them by number (I-10, I-11, …). New ideas go in
`prompt/IDEAS.md`.

## From student feedback, 2026-09-15 (practical chapters)

*All entries below were adopted in prompt v0.12 (2026-09-24); the marks under each heading say where. New ideas go under a new heading.*
...
**Adopted in v0.12 (§7e "Methods" block, closed by default with the type names in its summary; §7d Step column, each Step label linking to its entry).**
...
**Adopted in v0.12 (§7e last bullet, §5 structured data, Appendix A.6, §15/A.7).**
...
**Adopted in v0.12 (§7e; §7d glossary entries carry a numeric example).**
...
**Adopted in v0.12 (§13a summary; Appendix A.6).** The Answer line, Fast route, given values and step 1 stay visible; steps 2..n collapse.
...
**Adopted in v0.12 (§7e "Fast route"; §7d answer block order).**
...
**Adopted in v0.12 (§7e; §11b section 2; §3).**
...
**Adopted in v0.12 (§13a summary; Appendix A.4; §11 how-to).** Chosen form: one toggle in the always-visible bar (importance ≥ 3 only — a repetition arm would be defeated by I-12; secondary lines folded, step details closed), not a per-chapter view.
...
**Adopted in v0.12 (§0d; §15 hash check; §17).**
...
**Adopted in v0.12 (§0e rewritten; settings row; §19 notice).**
...
**Adopted in v0.12 (§5 record fields, §5a test with the exception as its step 2, §5b pass and build checks, §15).** The lessons are in §5a (kept record, not-duplicates, same-form definition, sub-question labels) and §5b (word similarity as an aid only). The exception is confirmed: it is part of the test, not a caveat.
