...
| `index.html` | The home page. Course cards are rendered from `courses.json`. |
| `scripts/` | `publish_page.py` copies a course's `out.html` to its pages folder and appends the analytics snippet; `build_course_index.py` regenerates `courses.json`. |
| `courses.json` | Generated by `scripts/build_course_index.py` from each course's `bank.json` (or a manual `S<n>/<CODE>/course.json`); a GitHub Action rebuilds it on push. Do not edit by hand. |
...
| `CLAUDE.md` | Map of the repository for AI agents: where state lives, the release loop, the traps. |
...
3. Run `python scripts/publish_page.py S<n>/<CODE>` to put the page at `S<n>/<CODE>/index.html`.
4. Run `python scripts/build_course_index.py` (or let the GitHub Action do it) so the home page picks it up.
